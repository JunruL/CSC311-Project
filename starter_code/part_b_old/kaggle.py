...
def predict_private_data(model, gender_vector, age_vector, train_data, lr, lamb, num_epoch):
    device = (torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'))
    ...
    save_private_test_csv(private_test, file_name=f'private_test_result_w_{lr}_{lamb}_{num_epoch}.csv')
...
def main():
    ...
    print(f'lr={lr}, lamb={lamb}, num_epoch={num_epoch}, best_valid_acc={best_valid_acc}, test_acc={test_acc}')

    predict_private_data(model, gender_vector, age_vector, zero_train_matrix, lr, lamb, num_epoch)
