...
class AutoEncoder(nn.Module):
    def __init__(self, num_student):
        ...
        self.q1 = nn.Linear(388, 1024)
        self.q2 = nn.Linear(1024, 256)
        self.q3 = nn.Linear(256, 64)

        self.h2 = nn.Linear(64+64, 256)
    ...
    def get_weight_norm(self):
        ...
        q_w_norm = torch.norm(self.q1.weight, 2) ** 2 +\
                   torch.norm(self.q2.weight, 2) ** 2 +\
                   torch.norm(self.q3.weight, 2) ** 2
        h_w_norm = torch.norm(self.h1.weight, 2) ** 2 +\
                   torch.norm(self.h2.weight, 2) ** 2
        return g_w_norm + q_w_norm + h_w_norm
...
def main():
    print(f'nn6.py, two encoders + one decoder (map to high dimension first)')
